...
In order to ensure the dispatcher is provided the correct `model.id`, we store the model's id on the node using the jQuery data property, `modelId`. This way, [the proper id can be used when notifying the dispatcher](https://github.com/AndrewHenderson/mariodux/blob/master/examples/todos/components/TodoList.js#L54).
```js
dispatch(toggleTodo(this.$el.data('modelId')));
```
